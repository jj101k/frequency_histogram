...
/**
 * This operates like a unique Map<F, number> with autogenerated results. F must
 * be convertible to string for this to work.
 *
 * This has optimisations for runs of identical values.
 *
 * @template F
 */
class UniqueValueMapper {
    ...
    #highestOut = 1
    /**
     * @type {{in: F, out: number} | undefined}
     */
    #last
    /**
     ...
     * @protected
     *
     * Generates a new out value, and stores it with reference to v for later
     * use.
     *
     * @param {F} v
     * @returns The new out value
     */
    add(v) {
        const out = this.#highestOut + 1
        this.#seen["" + v] = out
        this.#last = {in: v, out}
        this.#highestOut = out
        return out
    ...
    /**
     * If v has not previously been seen, this will return a number which has
     * not previously been seen. Otherwise, this will return the last number
     * which corresponded to v.
     *
     * @param {F} v
     * @returns
     */
    get(v) {
        if(!this.#last) {
            this.#last = {in: v, out: this.#highestOut}
            return this.#last.out
        } else if(this.#last.in === v) {
            return this.#last.out
        }
        let previous = this.#seen["" + v]
        if(previous === undefined) {
            return this.add(v)
        }
        this.#last = {in: v, out: previous}
        return previous
    }
}

/**
 * A value mapper optimised for runs of non-decreasing numbers.
 *
 * @extends {UniqueValueMapper<number>}
 */
class OptionallyIncreasingNumberValueMapper extends UniqueValueMapper {
    /**
     ...
     */
    get(v) {
        if(this.#highestIn === undefined || this.#highestIn < v) {
            this.#highestIn = v
            return this.add(v)
        }
        return super.get(v)
...
class StaticValueScaler extends ValueScaler {
    reset() {
        this.#uniqueValueMapper = new OptionallyIncreasingNumberValueMapper()
    }
    /**
     *
     */
    #uniqueValueMapper = new OptionallyIncreasingNumberValueMapper()
     ...
     */
    scale(d) {
        return this.#uniqueValueMapper.get(d) * this.scaleFactor
    }
}